...
trap_dispatch(struct Trapframe *tf)
{
  ...
  if (tf->tf_trapno == T_SYSCALL) {
    // syscallno eax, a1 edx, a2 ecx, a3 ebx, a4 edi, a5 esi -> eax
...
page_fault_handler(struct Trapframe *tf)
{
  ...
  if (curenv->env_pgfault_upcall) {
    ...
    uintptr_t dst = UXSTACKTOP - sizeof(struct UTrapframe);
    if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp <= UXSTACKTOP - 1)
      dst = tf->tf_esp - sizeof(uintptr_t) - sizeof(struct UTrapframe);
    utf = (struct UTrapframe *) dst;
    ...
    env_run(curenv);
    panic("page_fault_handler: return?");
  }
