...
boot_alloc(uint32_t n)
{
  ...
  result = nextfree;
  if (n > 0) {
    char *nextfree1 = nextfree;
    nextfree = nextfree + ROUNDUP(n, PGSIZE);
    
    if ((uint32_t) PADDR(nextfree) > npages * PGSIZE)
      panic("boot_alloc: is out of memory, trying to allocate %08x + %08x (%08x) = %08x, max size of %08x \n", (uint32_t) PADDR(nextfree1), n, ROUNDUP(n, PGSIZE), (uintptr_t) PADDR(nextfree), npages * PGSIZE);
  }
...
mem_init(void)
{
  ...
  // boot_map_region(kern_pgdir, KSTACKTOP - PTSIZE, PTSIZE - KSTKSIZE, NULL, PTE_W);
...
page_alloc(int alloc_flags)
{
  ...
  // if there is another page on free list
  if (result != NULL) {
    // remove from LL
    page_free_list = result->pp_link;
    result->pp_link = NULL;

    // cprintf("page_alloc -> PA %08x KVA %x\n", page2pa(result), page2kva(result));
...
// table and page directory entries.pte_t
//
pte_t *
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  // Find page directory entry base on index from vaddr
  pte_t *pte = pgdir + PDX(va);

  // if the PDE is not present, make it
  if (!(*pte & PTE_P)) {
    if (!create) return NULL;
    
    // allocate a page appropriately
    struct PageInfo *page = page_alloc(ALLOC_ZERO);
    if (page == NULL) return NULL;

    // prepare the allocated page
    page->pp_ref++;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  // for all requested blocks
  for (int i = 0; i < size; i += PGSIZE) {
    // fetch the pagetable entry
    pte_t *pte = pgdir_walk(pgdir, (void *) va + i, 1);
    assert(pte);

    // appy permission bits by resetting PTE
    *pte = (pa + i) | (perm | PTE_P);
  }
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
  ...
  // read / create pte for virtual addr
  ...
  // page_remove a page if it is already mapped
  if (pte && (*pte & PTE_P)) {
...
page_remove(pde_t *pgdir, void *va)
{
  ...
  // if a physical page is present
  if (pp && (*pte_store & PTE_P)) {
    // decrement ref count, free if necessary
    page_decref(pp);

    // set page table for va to 0
    *pte_store = 0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
  ...
  size_t size_rounded = ROUNDUP(pa + size, PGSIZE) - pa_rounded;
  
  assert(pa_rounded % PGSIZE == 0);
  assert(size_rounded % PGSIZE == 0);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  for (int i = ROUNDDOWN((uintptr_t) va, PGSIZE); (void*)i < ROUNDUP(va + len, PGSIZE); i += PGSIZE) {
    ...
    if (i >= ULIM || (env->env_pgdir[PDX(va)] & perm) != perm || !pte || (*pte & perm) != perm) {
